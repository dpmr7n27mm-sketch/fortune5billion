...
\
  const initGame = useCallback(() => \{\
    ...
    \
    // Initialize music player (will only work when deployed with actual audio file)\
    // Comment out or remove these lines if you want to test without music\
    try \{\
      if (!musicPlayerRef.current) \{\
        musicPlayerRef.current = createMusicPlayer();\
        // UPDATE THIS PATH when deploying to fortune5billion.com\
        musicPlayerRef.current.load('/nira-bgm.mp3');\
      \}\
      musicPlayerRef.current.play();\
    \} catch (e) \{\
      // Music file not available, continue without it\
    \}\
    \
...
\
  useEffect(() => \{\
    const handleKeyDown = (e) => \{\
      ...
      if (e.code === 'Space') \{\
        e.preventDefault();\
        if (gameState === 'start' || gameState === 'dead' || gameState === 'end') \{ initGame(); return; \}\
        if (frozen) return;\
        const g = gameRef.current;\
...
\
  useEffect(() => \{\
    const glitch = setInterval(() => \{ if (Math.random() > 0.7) \{ const chars = '\uc0\u9617 \u9618 \u9619 \u9608 \u9604 \u9600 \u926 \u937 \u936 \u916 '; setGlitchText('NOTHING IS REAL ANYMORE'.split('').map(c => Math.random() > 0.85 ? chars[Math.floor(Math.random() * chars.length)] : c).join('')); setTimeout(() => setGlitchText('NOTHING IS REAL ANYMORE'), 100); \}\}, 250);\
    return () => clearInterval(glitch);\
  \}, []);\
\
  const g = gameRef.current;\
  const FortuneCookie = (\{ screenX, y \}) => (<div style=\{\{ position: 'absolute', left: screenX, top: y - 20 \}\}><div style=\{\{ position: 'relative', width: 45, height: 35 \}\}><div style=\{\{ position: 'absolute', left: 0, width: 20, height: 30, background: 'linear-gradient(135deg, #2a2a2a 0%, #1a1a1a 50%, #0a0a0a 100%)', borderRadius: '50% 0 0 50%', border: '2px solid #444', boxShadow: '0 0 12px rgba(80,80,80,0.4)', transform: 'rotate(-15deg)' \}\} /><div style=\{\{ position: 'absolute', right: 0, width: 20, height: 30, background: 'linear-gradient(225deg, #2a2a2a 0%, #1a1a1a 50%, #0a0a0a 100%)', borderRadius: '0 50% 50% 0', border: '2px solid #444', boxShadow: '0 0 12px rgba(80,80,80,0.4)', transform: 'rotate(15deg)' \}\} /><div style=\{\{ position: 'absolute', left: 10, top: 11, width: 24, height: 6, background: '#f5f5f0', borderRadius: 1 \}\} /></div></div>);\
...
\
  if (gameState === 'start') return (<div onClick=\{initGame\} style=\{\{ width: '100%', height: '100vh', minHeight: 500, background: 'linear-gradient(180deg, #0a0a0a 0%, #151515 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', color: '#fff', cursor: 'pointer', position: 'relative' \}\}><div style=\{\{ fontSize: 11, color: '#555', letterSpacing: 3, marginBottom: 25 \}\}>FORTUNE5BILLION PRESENTS</div><div style=\{\{ position: 'relative', width: 70, height: 55, marginBottom: 25 \}\}><div style=\{\{ position: 'absolute', left: 0, width: 30, height: 45, background: 'linear-gradient(135deg, #2a2a2a 0%, #1a1a1a 50%, #0a0a0a 100%)', borderRadius: '50% 0 0 50%', border: '2px solid #444', boxShadow: '0 0 20px rgba(80,80,80,0.3)', transform: 'rotate(-15deg)' \}\} /><div style=\{\{ position: 'absolute', right: 0, width: 30, height: 45, background: 'linear-gradient(225deg, #2a2a2a 0%, #1a1a1a 50%, #0a0a0a 100%)', borderRadius: '0 50% 50% 0', border: '2px solid #444', boxShadow: '0 0 20px rgba(80,80,80,0.3)', transform: 'rotate(15deg)' \}\} /><div style=\{\{ position: 'absolute', left: 17, top: 18, width: 36, height: 10, background: '#f5f5f0', borderRadius: 2 \}\} /></div><h1 style=\{\{ fontSize: 42, fontWeight: 'bold', letterSpacing: 8, margin: 0, textShadow: '0 0 25px rgba(255,255,255,0.2)' \}\}>N.I.R.A.</h1><div style=\{\{ fontSize: 13, color: '#666', letterSpacing: 2, marginTop: 8, marginBottom: 35 \}\}>\{glitchText\}</div><div style=\{\{ padding: '14px 45px', fontSize: 13, border: '2px solid #fff', letterSpacing: 4 \}\}>TAP TO START</div><div style=\{\{ fontSize: 9, color: '#444', marginTop: 30, textAlign: 'center', lineHeight: 1.8 \}\}>COLLECT FORTUNES \'95 DODGE EXECUTIVES & CONTRACTS<br/>LOSE ALL COOKIES = GAME OVER</div><div style=\{\{ position: 'absolute', bottom: 20, textAlign: 'center' \}\} onClick=\{(e) => e.stopPropagation()\} onTouchStart=\{(e) => e.stopPropagation()\} onTouchEnd=\{(e) => e.stopPropagation()\}><a href="https://fortune5billion.com" target="_blank" rel="noopener noreferrer" onClick=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} onTouchEnd=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} style=\{\{ fontSize: 9, color: '#666', textDecoration: 'none', letterSpacing: 2, display: 'inline-block', padding: '12px 20px', cursor: 'pointer' \}\}>VISIT FORTUNE5BILLION.COM</a><div style=\{\{ fontSize: 8, color: '#333', marginTop: 8, letterSpacing: 1 \}\}>\'a9 2026 FORTUNE5BILLION INC. All Rights Reserved.</div></div></div>);\
  if (gameState === 'dead') return (<div style=\{\{ width: '100%', height: '100vh', minHeight: 500, background: 'linear-gradient(180deg, #0a0a0a 0%, #100000 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', color: '#fff', position: 'relative' \}\}><div style=\{\{ fontSize: 11, color: '#600', letterSpacing: 3, marginBottom: 20 \}\}>BYE BYE MASTERS</div><h1 style=\{\{ fontSize: 28, letterSpacing: 4, margin: 0, marginBottom: 15, color: '#ff4444' \}\}>THEY GOT YOU</h1><div style=\{\{ fontSize: 12, color: '#666', marginBottom: 30 \}\}>THE INDUSTRY WINS THIS ROUND</div><div onClick=\{initGame\} onTouchEnd=\{(e) => \{ e.preventDefault(); initGame(); \}\} style=\{\{ padding: '14px 35px', fontSize: 13, border: '2px solid #fff', letterSpacing: 3, cursor: 'pointer', marginBottom: 15 \}\}>TAP TO TRY AGAIN</div><a href="https://fortune5billion.com/support" target="_blank" rel="noopener noreferrer" onClick=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com/support', '_blank'); \}\} onTouchEnd=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com/support', '_blank'); \}\} style=\{\{ padding: '12px 25px', fontSize: 11, fontFamily: 'monospace', background: 'transparent', border: '1px solid #444', color: '#888', textDecoration: 'none', letterSpacing: 2, cursor: 'pointer', display: 'inline-block' \}\}>SUPPORT THE PRODUCER</a><div style=\{\{ position: 'absolute', bottom: 20, textAlign: 'center' \}\}><a href="https://fortune5billion.com" target="_blank" rel="noopener noreferrer" onClick=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} onTouchEnd=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} style=\{\{ fontSize: 9, color: '#666', textDecoration: 'none', letterSpacing: 2, display: 'inline-block', padding: '12px 20px', cursor: 'pointer' \}\}>VISIT FORTUNE5BILLION.COM</a><div style=\{\{ fontSize: 8, color: '#333', marginTop: 8, letterSpacing: 1 \}\}>\'a9 2026 FORTUNE5BILLION INC. All Rights Reserved.</div></div></div>);\
  if (gameState === 'end') return (<div style=\{\{ width: '100%', height: '100vh', minHeight: 500, background: 'linear-gradient(180deg, #0a0a0a 0%, #151515 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', color: '#fff', position: 'relative' \}\}><div style=\{\{ fontSize: 14, color: '#888', marginBottom: 30 \}\}>YOU KEPT YOUR FORTUNE</div><h1 style=\{\{ fontSize: 32, letterSpacing: 6, margin: 0, marginBottom: 15 \}\}>N.I.R.A. VOL 1</h1><div style=\{\{ fontSize: 12, color: '#555', textAlign: 'center', lineHeight: 1.8, marginBottom: 35 \}\}>AN AUDITORY EXPERIENCE DESIGNED BY<br /><span style=\{\{ color: '#fff', letterSpacing: 2 \}\}>FORTUNE5BILLION</span></div><a href="#" style=\{\{ padding: '14px 35px', fontSize: 13, fontFamily: 'monospace', background: '#fff', color: '#000', textDecoration: 'none', letterSpacing: 3, marginBottom: 12, display: 'inline-block' \}\}>STREAM VOL 1 NOW</a><button onClick=\{initGame\} onTouchEnd=\{(e) => \{ e.preventDefault(); initGame(); \}\} style=\{\{ padding: '10px 25px', fontSize: 11, fontFamily: 'monospace', background: 'transparent', border: '1px solid #444', color: '#444', cursor: 'pointer', letterSpacing: 2 \}\}>PLAY AGAIN</button><div style=\{\{ position: 'absolute', bottom: 20, textAlign: 'center' \}\}><a href="https://fortune5billion.com" target="_blank" rel="noopener noreferrer" onClick=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} onTouchEnd=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} style=\{\{ fontSize: 9, color: '#666', textDecoration: 'none', letterSpacing: 2, display: 'inline-block', padding: '12px 20px', cursor: 'pointer' \}\}>VISIT FORTUNE5BILLION.COM</a><div style=\{\{ fontSize: 8, color: '#333', marginTop: 8, letterSpacing: 1 \}\}>\'a9 2026 FORTUNE5BILLION INC. All Rights Reserved.</div></div></div>);\
\
  return (\
    <div onClick=\{() => \{ if (frozen) return; const g = gameRef.current; if (g.playerY >= GROUND - 5 && !g.isJumping) \{ g.isJumping = true; g.velocity = -780; playSound(audioCtxRef.current, 'jump'); \} \}\} style=\{\{ width: '100%', height: '100vh', minHeight: 500, background: hitFlash ? '#200000' : '#0a0a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', userSelect: 'none', cursor: 'pointer', transition: 'background 0.1s', position: 'relative' \}\}>\
      \{glitchBurst && (\
      ...
      <div style=\{\{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '90%', maxWidth: 600, padding: '10px 0', color: '#fff', fontSize: 14 \}\}><div style=\{\{ fontSize: 9, color: '#555', letterSpacing: 1 \}\}>TAP SCREEN TO JUMP</div><div style=\{\{ display: 'flex', alignItems: 'center', gap: 10, background: '#1a1a1a', padding: '6px 14px', border: '1px solid #333' \}\}><div style=\{\{ width: 14, height: 14, background: '#222', border: '2px solid #555', borderRadius: 2, transform: 'rotate(45deg)' \}\} /><span style=\{\{ color: g.collected >= GOAL ? '#4f4' : '#fff', fontSize: 16, fontWeight: 'bold', letterSpacing: 2 \}\}>\{g.collected\} / \{GOAL\}</span></div></div>\
      ...
      <button onClick=\{(e) => \{ e.stopPropagation(); initGame(); \}\} onTouchEnd=\{(e) => \{ e.stopPropagation(); \}\} style=\{\{ marginTop: 15, padding: '8px 20px', fontSize: 10, fontFamily: 'monospace', background: 'transparent', border: '1px solid #333', color: '#444', cursor: 'pointer', letterSpacing: 2 \}\}>RESTART (R)</button>\
      <div style=\{\{ position: 'absolute', bottom: 15, textAlign: 'center' \}\} onClick=\{(e) => e.stopPropagation()\} onTouchStart=\{(e) => e.stopPropagation()\} onTouchEnd=\{(e) => e.stopPropagation()\}><a href="https://fortune5billion.com" target="_blank" rel="noopener noreferrer" onClick=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} onTouchEnd=\{(e) => \{ e.stopPropagation(); window.open('https://fortune5billion.com', '_blank'); \}\} style=\{\{ fontSize: 9, color: '#444', textDecoration: 'none', letterSpacing: 2, display: 'inline-block', padding: '12px 20px', cursor: 'pointer' \}\}>VISIT FORTUNE5BILLION.COM</a><div style=\{\{ fontSize: 8, color: '#222', marginTop: 6, letterSpacing: 1 \}\}>\'a9 2026 FORTUNE5BILLION INC. All Rights Reserved.</div></div>\
    </div>\
  );\
\}}