  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.code === 'Space') {
        ...
        if (gameState === 'dead' || gameState === 'end') return; // Don't restart on space for these screens
        if (frozen) return;
        const g = gameRef.current;
    ...
    const handleTouch = (e) => {
      // Only auto-start on the start screen, not dead/end screens
      if (gameState === 'start') { initGame(); return; }
      if (gameState === 'dead' || gameState === 'end') return; // Don't restart - use buttons instead
      if (frozen) return;
      const g = gameRef.current;
